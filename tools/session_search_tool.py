...
def _get_session(db, session_id: str, session_scope: Optional[Dict[str, Any]]):
    if session_scope is None:
        return db.get_session(session_id)
    return db.get_session(session_id, session_scope=session_scope)


def _get_messages(db, session_id: str, session_scope: Optional[Dict[str, Any]]):
    if session_scope is None:
        return db.get_messages(session_id)
    return db.get_messages(session_id, session_scope=session_scope)
...
def _get_messages_around(
    ...
    if session_scope is None:
        return db.get_messages_around(session_id, around_message_id, window=window)
    return db.get_messages_around(
        session_id,
        around_message_id,
        window=window,
...
def _resolve_to_parent(
    ...
    while cur and cur not in visited:
        visited.add(cur)
        try:
            s = _get_session(db, cur, session_scope)
            if not s:
                break
...
def _read_session(
    ...
    try:
        meta = _get_session(db, session_id, session_scope) or {}
    except Exception as e:
    ...
    try:
        rows = _get_messages(db, session_id, session_scope)
    except Exception as e:
...
def _scroll(
    ...
    try:
        session_meta = _get_session(db, session_id, session_scope) or {}
    except Exception as e:
        logging.debug("get_session failed for %s: %s", session_id, e, exc_info=True)
        session_meta = {}
    if not session_meta:
    ...
    if not messages:
        ...
        if owning and owning != session_id:
            ...
            if a_root and o_root and a_root == o_root:
                try:
                    rebind_view = _get_messages_around(
                        db,
                        owning,
                        around_message_id,
                        window,
                    ...
                    if messages:
                        ...
                        try:
                            session_meta = _get_session(db, owning, session_scope) or session_meta
                        except Exception:
                            pass
                        session_id = owning
                except Exception as e:
...
def _title_match_result(
    ...
    try:
        session_meta = (
            _get_session(db, lineage_root, session_scope)
            or _get_session(db, session_id, session_scope)
            or {}
        )
    except Exception:
    ...
    try:
        messages = _get_messages(db, session_id, session_scope)
    except Exception:
...
def _discover(
    ...
    for lineage_root, match_info in seen_sessions.items():
        ...
        try:
            session_meta = _get_session(db, lineage_root, session_scope) or {}
        except Exception:
            session_meta = {}
