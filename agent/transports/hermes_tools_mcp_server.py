...
def _build_server() -> Any:
    ...
    description = (
        "Synchronize one external Kanban task by exact external_key or explicit "
        "task_id. dry_run must be explicit; apply requires expected_current_status."
    )
    try:
        mcp.add_tool(
            kanban_sync_external_task,
            name=KANBAN_EXTERNAL_SYNC_TOOL,
            description=description,
        )
    except TypeError:
        mcp.tool(
    ...
    logger.info(
        "hermes-tools MCP server registered %d/%d Hermes tools plus external sync",
        exposed_count,
        len(EXPOSED_TOOLS),
    )
    return mcp
