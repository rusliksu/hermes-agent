...
- [x] 2.3 Подключить выбранный source к `loadSessions`, overview poll refresh и post-delete/prune reload paths так, чтобы `total` и pagination приходили от server-side source filter.
...
- [x] 2.6 Убедиться, что delete/export/resume/rename/prune действия не меняют session data contract и продолжают работать с отфильтрованным списком.
...
- [x] 4.1 Добавить targeted frontend tests на default History+Telegram.
- [x] 4.2 Добавить targeted frontend tests на смену filter: reset page, reset selection, reset expanded row, server params.
- [x] 4.3 Добавить targeted frontend tests на badge/font class и normal-case behavior.
- [x] 4.4 Из каталога `web` запустить targeted frontend tests для `api.getSessions` и `SessionsPage` через поддерживаемый test runner/package script.
