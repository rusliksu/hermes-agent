## 1. Зафиксировать OpenSpec scope

- [x] 1.1 Создать repo-local change `restore-gurra-family-access` в существующем формате `openspec/changes`.
- [x] 1.2 Описать ordinary family DM access через explicit `telegram_allowed_user_ids` без реальных ID или private values.
- [x] 1.3 Зафиксировать owner-only admin, pairing, elevated actions и approvals.
- [x] 1.4 Зафиксировать unchanged две shared groups, fail-closed unknown/missing principals, redacted diagnostics, per-user isolation и отдельный live gate.

## 2. Подтвердить implementation boundaries

- [x] 2.1 Подтвердить git evidence, что family changes не модифицируют isolation files из privacy suite.
...
- [ ] 5.1 Подготовить private config patch только после отдельного explicit live gate.
- [ ] 5.2 Подготовить backup/rollback readiness только после отдельного explicit live gate.
- [ ] 5.3 Выполнить live patch/config/restart только после отдельного explicit live gate.
