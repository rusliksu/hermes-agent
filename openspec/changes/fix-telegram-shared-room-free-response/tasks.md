...
- [x] 5.1 Подготовить отдельный PR этой задачи только после реализации, тестов,
  проверки и строгой валидации. Доказательство: создан standalone draft PR
  #13 https://github.com/rusliksu/hermes-agent/pull/13 после реализации,
  тестов, review и strict validation; PR OPEN/DRAFT/MERGEABLE на head
  `1674a605...`.
- [x] 5.2 Перевод из `draft` в `ready` и `merge` выполнять только после проверки,
  актуального `diff`/`status` и подтверждённой готовности к `merge`.
  Доказательство: перед переводом из черновика и слиянием подтверждены чистое
  рабочее дерево задачи, ожидаемые ветка и SHA, `origin/main` не впереди, PR
  #13 открыт, остаётся черновиком, допускает слияние и не имеет конфликтов;
  отзывы, комментарии, незакрытые обсуждения и проверки отсутствуют; разница
  содержит только 7 ожидаемых файлов, `git diff --check` чист.
