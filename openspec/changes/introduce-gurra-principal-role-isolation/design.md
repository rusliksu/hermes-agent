...
- Зафиксировать roster/preflight baseline: provisioning 9 family profiles и назначение Юли в `family_sandbox` возможны только после private manually confirmed roster для всех девяти family transport identities.
- Зафиксировать capability границы для `owner`, `family_standard`, `family_sandbox`, `shared_room`, включая Юлин Docker sandbox без host mounts/owner credentials.
...
Оператор вручную подтверждает Юлину binding. Только она получает `family_sandbox`; остальные восемь family identities получают `family_standard`. Display name/username сохраняются максимум как redacted admin labels для операторского UI/audit и не являются authority. Любой missing, duplicate или ambiguous roster mapping блокирует profile provisioning, migration preflight и live cutover.
...
- `family_standard`: только личные memory/session search, документы, attachments, vision, public web, image/voice generation и reminders самому себе с server-bound `delivery_target`; без host shell, host filesystem, logged-in browser, arbitrary MCP, Wolfram MCP, delegation, private delivery к другим адресатам и cross-principal/profile search.
- `family_sandbox`: все возможности `family_standard` плюс собственный Docker workspace, isolated public browser, Wolfram MCP allowlist и delegation только внутри собственного `profile_id`; в первом rollout эта роль применяется к Юле.
- `shared_room`: room sessions/memory, документы, attachments, vision и public web только внутри room profile/scope; без private memory, private DM session search, cron, private delivery, shell, delegation, logged-in browser и cross-principal/profile search.
...
Решение: path-sensitive компоненты должны читать profile home через request-scoped context, а не через module/import-time `HERMES_HOME`. Любые `os.getenv` auth fallbacks на request path заменяются server-bound config/policy объектами. `session_search`, memory tools, memory hydration/commit и context-engine namespace получают profile/session scope из server-owned `ResolvedAccessContext`.

Это касается cached agents, callbacks, background tasks, cron jobs, delegation workers, compaction, reset/restart recovery и concurrent turns: при resume или delayed callback контекст берется из persisted server-bound metadata, а не угадывается по transport IDs или текущему process env.
...
3. Добавить family/shared role capability enforcement, self reminders, scoped secrets, Юлин Docker sandbox, isolated public browser, Wolfram allowlist и same-profile delegation.
...
6. Выполнить private roster/preflight validation: 9 family transport identities mapped to opaque `principal_id -> role_id -> profile_id`, Юлина binding manually confirmed as only `family_sandbox`, остальные восемь `family_standard`, no missing/duplicate/ambiguous mappings, display labels redacted and non-authoritative.
