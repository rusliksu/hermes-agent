...
Подтвержденная дельта 2026-07-24: идентификаторы ролей остаются ровно `owner`, `family_standard`, `family_sandbox`, `shared_room`. `family_standard` по умолчанию остается без Wolfram/arbitrary MCP, но private manually confirmed roster может назначить ровно трем private family `PrincipalBinding` literal capability `wolfram`. Это per-principal capability grant, а не пятая роль, не отдельная роль под человека и не special-case principal. Human labels для этих трех grants никогда не попадают в artifacts; username/display-name не являются authority, proof или источником назначения. Назначение действует только для точного настроенного allowlist Wolfram MCP в собственном family profile/context каждой binding и не дает terminal, host filesystem, browser, delegation, arbitrary MCP, cross-profile search/data, private/cross-profile delivery или foreign delivery. `family_sandbox` сохраняет свой Wolfram allowlist независимо; shared-room configured MCP/cron policy из предыдущей дельты не меняется.
...
- Зафиксировать roster/preflight baseline: provisioning 9 family profiles, одна manually confirmed `family_sandbox` binding и ровно три private family `PrincipalBinding` с literal capability `wolfram` возможны только после private manually confirmed roster для всех девяти family transport identities.
- Зафиксировать capability границы для `owner`, `family_standard`, `family_sandbox`, `shared_room`, включая Docker sandbox для `family_sandbox` без host mounts/owner credentials, private per-principal `wolfram` capability для ровно трех family `PrincipalBinding` и полный серверно настроенный Telegram tool profile для typed shared rooms без private/cross-room/owner fallback.
...
Оператор вручную подтверждает одну sandbox binding. Только она получает `family_sandbox`; остальные восемь family identities получают `family_standard`. Тот же private roster вручную подтверждает ровно три private family `PrincipalBinding` с literal capability `wolfram`; эти capability assignments не меняют role ID, не создают пятую роль и не делают одного человека special. Username/display-name сохраняются максимум как redacted admin labels для операторского UI/audit, не являются authority и не попадают в artifacts как human labels для Wolfram grants. Любой missing, duplicate или ambiguous roster mapping, включая ambiguous `wolfram` capability assignment, блокирует profile provisioning, migration preflight и live cutover.
...
- `family_standard`: только личные memory/session search, документы, attachments, vision, public web, image/voice generation и reminders самому себе с server-bound `delivery_target`; без host shell, host filesystem, logged-in browser, arbitrary MCP, delegation, private delivery к другим адресатам и cross-principal/profile search. Wolfram MCP по умолчанию deny; только ровно три manually confirmed private family `PrincipalBinding` с literal capability `wolfram` могут использовать точный настроенный allowlist Wolfram MCP внутри собственного family profile/context, без human labels в artifacts и без authority из username/display-name.
...
3. Добавить family/shared role capability enforcement, self reminders, shared-room same-room cron delivery, scoped secrets, Docker sandbox для `family_sandbox`, isolated public browser, sandbox Wolfram allowlist, private `wolfram` capability для трех family `PrincipalBinding` и same-profile delegation.
...
6. Выполнить private roster/preflight validation: 9 family transport identities mapped to opaque `principal_id -> role_id -> profile_id`, одна binding manually confirmed as only `family_sandbox`, остальные восемь `family_standard`, ровно три private family `PrincipalBinding` с literal capability `wolfram`, no missing/duplicate/ambiguous mappings, username/display-name redacted and non-authoritative, no human labels in artifacts.
