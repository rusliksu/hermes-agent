...
- Resolve confirmed DM/room identities to `owner`, equal-capability `family`, or `shared_room`; accept old family role IDs only as migration aliases and deny everything else before model/session/tools.
...
- Cover Руслан(owner), all nine `family` principals, legacy role aliases, two rooms(shared_room), unknown, malformed, mismatched Telegram IDs and missing profile.
